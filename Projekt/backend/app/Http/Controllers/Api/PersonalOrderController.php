    ...
    try {
        ...
        if ($menuItem->soup) {
            ...
            if (!empty($commonAllergens)) {
                $warnings[] = [
                    'meal' => 'Leves',
                    'meal_id' => $menuItem->soup,
                    'meal_name' => $menuItem->soupMeal->mealName ?? 'Leves',
                    'allergen_ids' => $commonAllergens
                ];
            }
        }
        ...
        if ($menuItem->optionA) {
            ...
            if (!empty($commonAllergens)) {
                $warnings[] = [
                    'meal' => 'A opció',
                    'meal_id' => $menuItem->optionA,
                    'meal_name' => $menuItem->optionAMeal->mealName ?? 'A opció',
                    'allergen_ids' => $commonAllergens
                ];
            }
        }
        ...
        if ($menuItem->optionB) {
            ...
            if (!empty($commonAllergens)) {
                $warnings[] = [
                    'meal' => 'B opció',
                    'meal_id' => $menuItem->optionB,
                    'meal_name' => $menuItem->optionBMeal->mealName ?? 'B opció',
                    'allergen_ids' => $commonAllergens
                ];
            }
        }
        ...
        if ($menuItem->other) {
            ...
            if (!empty($commonAllergens)) {
                $warnings[] = [
                    'meal' => 'Egyéb',
                    'meal_id' => $menuItem->other,
                    'meal_name' => $menuItem->otherMeal->mealName ?? 'Egyéb',
                    'allergen_ids' => $commonAllergens
                ];
            }
        }
        ...
        Log::error('Allergén ellenőrzés hiba', ['error' => $e->getMessage()]);
        return [];
    }
}

    private function getMealAllergens($mealId)
    {
        try {
            $meal = \App\Models\Meal::with(['ingredients.allergens'])->find($mealId);
            ...
            return array_unique($allergenIds);
            
        } catch (\Exception $e) {
            Log::error('Étel allergének lekérése hiba', ['error' => $e->getMessage()]);
            return [];
        }
    }
...
public function reactivateOrder(Request $request, $id)
{
    ...
    try {
        ...
        // Nincs-e másik aktív rendelés ugyanarra a napra
 ...
 */
    ...
    private function canCancelOrder($order)
    {
        try {
            ...
            $deadline = $orderDate->copy()->setTime(10, 0, 0);
            //$deadline = $orderDate->copy()->subDay()->setTime(10, 0, 0);

            
            // Ha a mai nap a rendelés napja, és még nem múlt el 10:00
            if ($now->toDateString() === $orderDate->toDateString()) {
                return $now <= $deadline;
            }
            
            // Ha a rendelés napja a jövőben van,lemondható
    ...
    public function cancel($id)
    {
        ...
        try {
            ...
            if (!$this->canCancelOrder($order)) {
                ...
                return response()->json([
                    'success' => false,
                    'message' => 'A lemondási határidő lejárt. Lemondás csak 10:00-ig lehetséges.',
